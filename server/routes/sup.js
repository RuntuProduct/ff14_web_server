...
router.delete('/:_id', function(req, res, next) {
  ...
  Sup.remove({_id: _id}, function(err, suc) {
    if(err) console.log(err);
...
router.patch('/:_id', function(req, res, next) {
  ...
  Sup.findById(_id, function(err, fUser) {
    if(err) return console.log(err);
    delete fUser._id;   // 取出主键
    // 更新可配置属性
    buildGood(getUser, fUser);
    Sup.update({_id: _id}, fUser, function(err, suc) {
      if(err) console.log(err);
...
router.post('/add', function(req, res, next) {
  let getSup = req.body;
  console.log(getSup);
  if(!getSup.account){return dealRes(res, 1, 'need account!')}
  if(!getSup.pwd){return dealRes(res, 1, 'need pwd!')}
  if(getSup.disabled == undefined){return dealRes(res, 1, 'need disabled!')}
  let newSup = Sup({
    account: getSup.account,
    pwd: getSup.pwd,
    disabled: getSup.disabled
  });
  newSup.save(function(err, suc){
    if(err){
      console.log(err)
      if(err.code == 11000){
...
router.patch('/add/:_id', function(req, res, next) {
  ...
  Sup.findById(_id, function(err, suc){
    ...
    if(!suc || suc == null){
      ...
      delete suc._id; // 剔除主键
      // 更新可配置属性
      if(getSup.pwd){suc.pwd = getSup.pwd}
      if(getSup.disabled){suc.disabled = getSup.disabled}
      Sup.update({_id: _id}, suc, function(err, suc){
        if(err){return console.log(err)}
